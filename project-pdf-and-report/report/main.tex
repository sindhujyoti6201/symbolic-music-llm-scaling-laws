...
We use the Lakh MIDI Dataset (LMD-matched) \cite{raffel2016lakh}, which contains approximately 116,000 MIDI files matched to entries in the Million Song Dataset. For this study, we processed a subset of 10,000 MIDI files to balance computational requirements with dataset diversity.
...
\begin{enumerate}
    ...
    \item \textbf{Vocabulary Building}: A vocabulary is built from training data with a minimum token frequency threshold of 2. This results in a vocabulary size of 143 tokens.
    ...
    \begin{itemize}
        \item Training set: 98\% (359 sequences, 1,165,472 tokens)
        \item Validation set: 1\% (3 sequences, 8,062 tokens)
        \item Test set: 1\% (5 sequences, 18,434 tokens)
    \end{itemize}
\end{enumerate}
...
After processing 10,000 MIDI files, we obtained:
\begin{itemize}
    \item Successfully converted: 9,673 ABC files (96.7\% success rate)
    \item After filtering: 367 sequences meeting length requirements
    \item Vocabulary size: 143 tokens (5 special tokens + 138 regular tokens)
    \item Average sequence length: 3,291 tokens
    \item Total tokens: 1,191,968 tokens across all splits
\end{itemize}

The dataset split resulted in:
\begin{itemize}
    \item Training: 359 sequences (1,165,472 tokens, 98\%)
    \item Validation: 3 sequences (8,062 tokens, 1\%)
    \item Test: 5 sequences (18,434 tokens, 1\%)
\end{itemize}
